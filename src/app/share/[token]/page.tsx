...
export async function generateMetadata({
  ...
  const title = payload
    ? `MOSS60 Share · ${payload.metadata.id}`
    : "MOSS60 Share";
...
export default async function Moss60SharePage({ params }: SharePageProps) {
  enforceChildSafeServerRoute("/share/[token]");
    ...
    return (
      <main className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
        <p className="text-sm text-red-300">
          Invalid or corrupted share payload.
        </p>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
      <section className="max-w-2xl mx-auto rounded-xl border border-zinc-800 bg-zinc-900/70 p-6 space-y-4">
        ...
        <p className="text-sm text-zinc-400">
          Read-only share view with verifiable metadata payload.
        </p>
        ...
        <dl className="grid grid-cols-[130px_1fr] gap-y-2 text-sm">
          ...
          <dd>{payload.metadata.id}</dd>
          <dt className="text-zinc-500">Theme</dt>
          <dd>{payload.metadata.scheme}</dd>
          <dt className="text-zinc-500">Variant</dt>
          <dd>{payload.metadata.variant}</dd>
          <dt className="text-zinc-500">Projection</dt>
          <dd>{payload.metadata.projection}</dd>
          <dt className="text-zinc-500">Seed</dt>
          <dd>{payload.metadata.seed || "—"}</dd>
          <dt className="text-zinc-500">Digest</dt>
          <dd className="font-mono text-xs break-all">{payload.digest}</dd>
        </dl>
