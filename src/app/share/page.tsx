...
export const metadata = {
  ...
  description: 'Earn rewards through play, care, and exploration. Share verifiable achievements with the world.',
};
...
const HOW_TO_EARN = [
  ...
  },
  {
    ...
    description: 'Reach new evolution stages (NEURO → QUANTUM → SPECIATION) and map the Vimana grid to unlock rare cosmetics.',
...
const REWARD_TIERS = [
  { tier: 'Bronze', color: 'border-orange-700/50 bg-orange-950/30 text-orange-300', essence: 10,  desc: 'First milestones — care, first battle, first scan.' },
  { tier: 'Silver', color: 'border-zinc-500/50 bg-zinc-900/50 text-zinc-300',        essence: 25,  desc: 'Sustained effort — 25 battles, 50 samples collected, streak runs.' },
  { tier: 'Gold',   color: 'border-yellow-500/50 bg-yellow-950/30 text-yellow-300',  essence: 50,  desc: 'Mastery — 100 battles, QUANTUM stage, resolve 10 anomalies.' },
  { tier: 'Platinum', color: 'border-cyan-500/50 bg-cyan-950/30 text-cyan-300',      essence: 100, desc: 'Legendary — perfect care, unstoppable streak, full map, speciation.' },
];
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-6 space-y-8">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <section className="space-y-4">
          <h2 className="text-base font-semibold text-zinc-200">How to earn rewards</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            {HOW_TO_EARN.map(({ emoji, title, description, route, cta }) => (
              <div key={title} className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 space-y-2 flex flex-col">
                <div className="text-2xl">{emoji}</div>
                <h3 className="text-sm font-semibold text-zinc-100">{title}</h3>
                <p className="text-xs text-zinc-400 leading-relaxed flex-1">{description}</p>
                <Link
                  href={route}
        ...
        {/* Achievement tiers */}
        <section className="space-y-4">
          <h2 className="text-base font-semibold text-zinc-200">Achievement tiers &amp; Essence</h2>
          <p className="text-xs text-zinc-500">Each unlocked achievement awards Essence instantly based on its tier.</p>
          <div className="grid gap-2 sm:grid-cols-2">
            {REWARD_TIERS.map(({ tier, color, essence, desc }) => (
              <div key={tier} className={`rounded-xl border p-4 space-y-1 ${color}`}>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold">{tier}</span>
                  <span className="font-mono text-xs font-bold">+{essence} Essence</span>
                </div>
