...
const AUDIENCE_DESCRIPTIONS: Record<PlanAudience, string> = {
  consumer:
    "A companion that grows with you — wellness, play, and personality.",
  educator:
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-10 text-zinc-100">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Privacy-first. No ads, no data harvesting. Billing integration coming
          soon.
        </p>
      </div>
      ...
      <div className="mb-4 flex justify-center">
        <div className="inline-flex rounded-xl border border-slate-700 bg-slate-900/80 p-1 gap-1">
          {(["consumer", "educator", "institution"] as PlanAudience[]).map(
            (a) => (
              <button
                key={a}
                type="button"
      ...
      {/* Plan Cards */}
      <div
        className={`grid gap-6 ${audiencePlans.length === 1 ? "max-w-sm mx-auto" : "md:grid-cols-2"}`}
      >
      ...
      <section className="mt-16 rounded-2xl border border-slate-800 bg-slate-900/50 p-6 md:p-8">
        ...
        <p className="mb-5 text-xs text-zinc-500">
          Three parallel paths — each designed to sustain the mission without
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-violet-500/20 bg-violet-950/20 p-4">
            ...
            <p className="mt-1 text-xs leading-relaxed text-zinc-400">
              $4.99/mo per person. Full pet experience — all addons, dream
              journal, wellness sync, advanced genome tools. Zero class
              management.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-950/20 p-4">
            ...
            <p className="mt-1 text-xs leading-relaxed text-zinc-400">
              Limited-edition addon drops, creator revenue share, and a
              community marketplace for curriculum add-ons. Drop pricing from
              $1.99.
            </p>
          </div>
        </div>
      ...
      <p className="mt-6 text-center text-xs text-zinc-700 max-w-md mx-auto leading-relaxed">
        On the horizon: district partnerships, gifting, and a parent/guardian
        tier. Built by people who care about quality of life.
      </p>
    </main>
  );
}
