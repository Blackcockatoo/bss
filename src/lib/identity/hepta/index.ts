...
export async function heptaEncode42(
  ...
  if (data30.length !== ECC_CONSTANTS.DATA_LENGTH) {
    throw new Error(
      `packPayload returned ${data30.length} digits, expected ${ECC_CONSTANTS.DATA_LENGTH}`,
    );
  }
...
export function createHeptaPayload(
  ...
  const minutes = Math.floor((options.now ?? Date.now()) / 60000) % 8192;
  const nonce = Math.floor((options.random ?? Math.random)() * 16384);
...
export async function heptaEncode42V2(
  ...
  if (data30.length !== ECC_V2_CONSTANTS.DATA_LENGTH) {
    throw new Error(
      `packPayload returned ${data30.length} digits, expected ${ECC_V2_CONSTANTS.DATA_LENGTH}`,
    );
  }
