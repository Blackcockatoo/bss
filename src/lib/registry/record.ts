 ...
 * v1 is the legacy passthrough; v2 is the chamber-aware all-180-locus fold
 * used by new registrations. Existing pets keep the version they were born
 * with so their appearance never silently changes.
 */
export const PROJECTION_VERSION_V1 = SRI_YANTRA_PROJECTION_VERSION_V1;
export const PROJECTION_VERSION_V2 = SRI_YANTRA_PROJECTION_VERSION_V2;
...
export interface RegistrationProofV1 {
  version: "registration-proof/v1";
...
export interface PetRecordV2 {
  ...
  /** Public-key proof used to verify MP2 packets on a different device. */
  registrationProof: RegistrationProofV1 | null;
...
function isRegistrationProof(value: unknown): value is RegistrationProofV1 {
  ...
  return (
    proof.version === "registration-proof/v1" &&
    proof.algorithm === "ECDSA-P256-SHA256" &&
    typeof proof.publicKeyJwk === "object" &&
    proof.publicKeyJwk !== null &&
...
export function isPetRecordV2(value: unknown): value is PetRecordV2 {
  ...
  return (
    ...
    record.vitals !== null &&
    (record.registrationProof === undefined ||
      record.registrationProof === null ||
      isRegistrationProof(record.registrationProof)) &&
