...
export interface PetRepository {
   ...
   */
  ensureRegisteredPet(
    options?: EnsureRegisteredPetOptions,
  ): Promise<PetRecordV2>;
}
  ...
  return {
    ...
    async ensureRegisteredPet(options = {}) {
      ...
      if (active) {
        ...
        if (!active.registrationProof) {
          active.registrationProof = await createRegistrationProof(active);
          changed = true;
        }
        if (changed) await this.saveRecord(active);
        return active;
      }
