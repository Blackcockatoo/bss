...
describe("buildPetRecord", () => {
  it("assembles a complete, valid record for a fresh genesis pet", async () => {
    ...
    expect(record.projectionVersion).toBe(PROJECTION_VERSION_V2);
    ...
    expect(record.registrationProof?.algorithm).toBe("ECDSA-P256-SHA256");
    await expect(verifyRegistrationProof(record)).resolves.toBe(true);
    expect(record.petId).toMatch(/^mp2-[0-9a-f]{16}$/);
  });

  it("detects immutable lineage tampering through the portable proof", async () => {
    const record = await buildPetRecord({ random: seededRandom(17), hmacKey });
    const tampered = {
      ...record,
      lineage: { ...record.lineage, generation: record.lineage.generation + 1 },
    };
    await expect(verifyRegistrationProof(tampered)).resolves.toBe(false);
  });
  ...
  it("mints the same identity for the same genome every time", async () => {
    const first = await buildPetRecord({
      genome: fixedDecimalGenome(),
      hmacKey,
    });
    const second = await buildPetRecord({
      genome: fixedDecimalGenome(),
      hmacKey,
    });
...
describe("deriveGeometryProjectionV2", () => {
  it("changes the engine-facing region packet for every source locus", async () => {
    const source = fixedDecimalGenome();
    const baseline = await deriveGeometryProjectionV2(source);
    for (let flatIndex = 0; flatIndex < 180; flatIndex += 1) {
      const changed = structuredClone(source);
      const strand = [changed.red60, changed.blue60, changed.black60][
        Math.floor(flatIndex / 60)
      ];
      const index = flatIndex % 60;
      strand[index] = (strand[index] + 1) % 10;
      const projected = await deriveGeometryProjectionV2(changed);
      expect(projected.fingerprint).not.toBe(baseline.fingerprint);
    }
  });
});
