...
function breedWithMutation(
  ...
  const genesToMutate = Math.floor(180 * mutationRate);
  ...
  for (let i = 0; i < genesToMutate; i++) {
    let flatPosition = Math.floor(rng() * 180);
    while (used.has(flatPosition)) {
      flatPosition = (flatPosition + 1) % 180;
    }
    used.add(flatPosition);
    const position = flatPosition % 60;
    const arrayChoice = Math.floor(flatPosition / 60);
    const target = [offspring.red60, offspring.blue60, offspring.black60][
      arrayChoice
    ];
    let newValue = Math.floor(rng() * radix);
    if (newValue === target[position]) newValue = (newValue + 1) % radix;
...
export function predictOffspring(
  ...
  const confidence = 50 + similarity / 2; // More similar = more predictable

  return { possibleTraits, confidence };
}

export {
  BREEDING_PACKET_PROTOCOL,
  BREEDING_RULESET_VERSION,
  breedRegisteredPets,
  createBreedingPacket,
  evaluateCrownwheel,
  isCrownwheelEdge,
  registerOffspring,
  verifyBreedingPacket,
  type BreedingPacketV1,
  type ChamberProvenance,
  type CrownwheelCompatibility,
  type CrownwheelOutcome,
