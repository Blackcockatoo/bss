...
export function StudentDNACard({
  profile,
  compact = false,
  showXP = false,
  showBadges = false,
  ...
  const unlockedAchievements = eduAchievements.filter(
    (a) => a.unlockedAt !== null,
  );
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span
          className="text-sm"
          title={`Your Pattern: ${profile.learningSymbol}`}
        >
          {symbolChar}
        </span>
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-2xl" title={profile.learningSymbol}>
            {symbolChar}
          </span>
          <div>
...
function AffinityBar({
  ...
  if (entries.length === 0) {
    return (
      <div className="rounded-full bg-slate-800" style={{ width, height }} />
    );
  }
