...
export const PetProfilePanel: React.FC<PetProfilePanelProps> = ({
  petId: legacyPetId = "auralia-default",
  petName: legacyPetName = "Auralia",
  onEditModeChange,
  editMode = false,
  record = null,
}) => {
  const [coatOfArms, setCoatOfArms] = useState<CoatOfArms | null>(null);
  const [activeTab, setActiveTab] = useState<"coat" | "keys" | "addons">(
    "coat",
  );
  ...
  const identityEmail =
    profile.email.trim() || "Add email on the Identity page";
    ...
    <div className="bg-slate-900/95 border border-slate-700 rounded-lg overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-purple-900/50 to-blue-900/50 px-4 py-3 border-b border-slate-700">
        ...
        <p className="text-xs text-slate-400 mt-1">
          Lineage, Identity & Customization
        </p>
      </div>

      {record?.heptaProfile && (
        <div className="border-b border-cyan-900/50 bg-cyan-950/20 px-4 py-3">
          <p className="text-[0.65rem] uppercase tracking-[0.2em] text-cyan-300/70">
            Registered Character
          </p>
          <p className="mt-1 text-sm font-semibold capitalize text-white">
            {record.heptaProfile.temperament}
          </p>
          <p className="mt-1 text-xs text-slate-400">
            {record.heptaProfile.dominantAxis} /{" "}
            {record.heptaProfile.secondaryAxis}
      ...
      <div className="p-4">
        ...
        {activeTab === "coat" && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500 text-center">
              Your coat of arms is a unique heraldic identifier for your pet
              lineage
            </p>
          </div>
        )}
        ...
        {/* Crypto Keys Tab */}
        {activeTab === "keys" && <CryptoKeyDisplay showPrivate={true} />}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-sm text-slate-400">
                Manage your addons from the Inventory panel
              </p>
