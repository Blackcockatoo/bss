 ...
 * behaviour layers. Every field passes through `sanitizeBodySpec`, so the v3
 * schema (including mischief/calm/fierce expressions) is accepted natively.
 */
export function importBodyForgeTransfer(value: unknown): BodySpec | null {
  if (
    !isPlainObject(value) ||
    value.version !== 1 ||
    value.kind !== "bss.body-forge-transfer" ||
  ...
  return sanitizeBodySpec({
    ...
    eyeHeight: 104,
    pupilSize: isFiniteNumber(body.eyes)
      ? clamp(body.eyes * 0.42, 2, 10)
      : undefined,
    gazeX: body.gazeX,
    gazeY: 0,
    mouthWidth: 30,
    mouthHeight: 11,
    wingSpread: isFiniteNumber(body.wing)
      ? clamp(body.wing / 82, 0.2, 1.65)
      : undefined,
    ...
    breathe: 0.035,
    animationSpeed: 1,
    // Rich v3 material/aura/emotion channels transfer when present; the
    // sanitiser fills safe defaults when the standalone forge omits them.
    auraStyle: body.auraStyle,
    auraMotion: body.auraMotion,
    auraDensity: body.auraDensity,
    auraRadius: body.auraRadius,
    auraSpeed: body.auraSpeed,
    auraTurbulence: body.auraTurbulence,
    auraDimension: body.auraDimension,
    auraColor: body.auraColor,
    auraSecondary: body.auraSecondary,
    emotionIndex: body.emotionIndex,
    features,
  });
}
...
export function decodeBodyForgeTransfer(encoded: string): BodySpec | null {
  try {
    ...
    return importBodyForgeTransfer(
