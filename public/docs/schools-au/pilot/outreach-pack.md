# MetaPet School Outreach Pack

Use this pack for the first principal or pilot-lead conversation.

## Principal Email Template

Subject: MetaPet School pilot pack and readiness verdict for review

Hello [Name],

I am sharing `MetaPet School`, a teacher-led, low-data classroom pilot for Years 3-6. The current recommendation is a constrained single-school pilot: `1 school`, `1 class`, `1 teacher`, with a stop/go review after the first fortnight. The school version is bounded: alias-based classroom use, no student accounts, no social features, and a governance pack designed for pilot review.

For a first pass, I recommend starting with:

1. Pilot Readiness Verdict v2
2. Overview and Alignment
3. Staff Briefing
4. Pilot Prospectus
5. What MetaPet School Is / Is Not

If useful, I can also share the privacy and safety pack for ICT or wellbeing review.

Regards,
[Name]

## First Meeting Agenda

1. Updated pilot readiness verdict and constrained scope
2. Problem and classroom use case
3. Age group and timetable fit
4. Product boundaries
5. Privacy and safety posture
6. Questions, blockers, and next steps

## What To Review First

### Principals and pilot leads

- Pilot Readiness Verdict v2
- Overview and Alignment
- Staff Briefing
- Pilot Prospectus
- School Partner Checklist
- What MetaPet School Is / Is Not

### ICT/privacy reviewers

- Privacy and Implementation Note
- Privacy Policy
- Data Flow Diagram
- Data Inventory
- ICT/Privacy Review Checklist

### Teachers

- Teacher Guide
- Lesson Cards
- Assessment and Reflection
- Teacher Dry-Run Checklist

### Families

- Parent Note
- Parent/Carer Privacy Notice
- Family Participation Protocol
