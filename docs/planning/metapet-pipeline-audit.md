...
Date: 2026-07-19
Branch: `claude/metapet-pipeline-audit-ed6vme`
Baseline: `main` @ `b9b8844` (post PR #156)
Status: audit only — no source code changed
...
- [ ] Fresh install creates or migrates one real registered pet.
- [ ] Reload restores the identical genome, personality, and geometry.
- [ ] Geometry never receives zero/default strands in normal production.
- [ ] Every one of 180 loci has tested influence on the geometry projection.
- [ ] Two contrasting genomes are unmistakably different visually.
- [ ] Two contrasting Hepta personalities behave noticeably differently.
- [ ] Geometry movement occurs at pet-specific deterministic intervals.
- [ ] Same breeding packet produces the same child.
- [ ] New conception nonce produces a different sibling.
- [ ] Non-mutated child loci always come from a declared parent.
- [ ] Preview and registered offspring are identical.
- [ ] All 892 existing tests plus the new integration/property tests pass.
- [ ] The protected Sri Yantra sprite checksum remains unchanged.
